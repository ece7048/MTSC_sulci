...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        self,
        img_size: Union[Sequence[int],int] = int,
        in_channels:Optional[int]= int,
        out_channels: Optional[int]=int,
        depths: Sequence[int] = (2, 2, 2, 2),
        num_heads: Sequence[int] = (3, 6, 12, 24),
        feature_size: Optional[int] = 24,
        norm_name: Union[tuple,str] = "instance",
        drop_rate: Optional[float] = 0.0,
        attn_drop_rate: Optional[float] = 0.0,
        dropout_path_rate: Optional[float] = 0.0,
    ...
    def forward(self, x_in):
        ...
        if self.roi_size<64:
            ...
            enc3 = self.encoder9(hidden_states_out[3])
            dec2 = self.decoder4(enc3,hidden_states_out[2])
            dec1 = self.decoder3(dec2,enc2)
            dec0 = self.decoder2(dec1, enc1)
            o = self.decoder1(dec0, enc0)
            logits = self.out(o)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        del attn, relative_position_bias
        return x
...
class Extend(nn.Module):
    def __init__(self, nclass):
        super().__init__()
        self.nclass=nclass
        self.out=UnetOutBlock(spatial_dims=3, in_channels=1, out_channels=self.nclass)
        self.to_out = nn.Linear(nclass, 2, bias = False)
...
class ExtendAttention(nn.Module):
    def __init__(self, nclass):
        super().__init__()
        self.nclass=nclass
        emd=int(64)
        self.multihead_attn = nn.MultiheadAttention(emd, 2)
        self.out=UnetOutBlock(spatial_dims=3, in_channels=1, out_channels=self.nclass)
        self.to_out = nn.Linear(nclass, 2, bias = False)
