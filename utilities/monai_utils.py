...
def sliding_window_inference(
    inputs: Union[torch.Tensor , MetaTensor],
    ...
    cval: float = 0.0,
    sw_device: Union[torch.device ,str] = None,
    ...
    context_in:Union[torch.Tensor , MetaTensor]=None,
    *args: Any,
    **kwargs: Any,
    ...
    for slice_g in tqdm(windows_range) if progress else windows_range:
        ...
        if sw_batch_size > 1:
            ...
            if context_in !=None:
                win_data2 = torch.cat([context_in[win_slice2] for win_slice2 in unravel_slice]).to(sw_device)
        else:
            win_data = inputs[unravel_slice[0]].to(sw_device)
            if context_in !=None:
                win_data2 = context_in[unravel_slice[0]].to(sw_device)
        if with_coord:
            if context_in !=None:
                seg_prob_out,classes = predictor(win_data, unravel_slice, context=win_data2, *args, **kwargs)  # batched patch
            else:
                seg_prob_out,classes = predictor(win_data, unravel_slice, *args, **kwargs)  # batched patch
            clas_total.append(classes)
        else:
            if context_in !=None:
                seg_prob_out,classes = predictor(win_data, context=win_data2, *args, **kwargs)  # batched patch
            else:
                seg_prob_out,classes = predictor(win_data, *args, **kwargs)  # batched patch
            clas_total.append(classes)
    ...
    clas_total=torch.cat(clas_total,dim=0)
    clas_total=torch.reshape(clas_total,(-1,batch_size,clas_total.shape[1]))
    class_=torch.argmax(clas_total,dim=0)
    class_=class_.squeeze()
    return final_output, class_  # type: ignore
